...
function ProductDetails(props) {
    ...
    const getSingleProduct = () => {
        ...
        axios.get(`http://localhost:4000/product/${id}`, {
            ...
            setProductDetails({ ...res.data?.info, ...res.data?.metaData, similarProduct: res.data?.similarProduct })
        })
    }
    ...
    return (
        <>
            ...
            {Object.keys(ProductDetails).length > 0 && <div className="product-details">
                <div className="container product-desc d-flex align-items-center justify-content-around mt-4">
                    <div className="row product-img">
                        <div className="product-image">
                            <img src={imgDefault ? `http://localhost:4000/${imgDefault}` : `http://localhost:4000/${ProductDetails?.images[0]}`} alt="" />
                        </div>
                        <div className="diff-product-image d-flex align-items-center justify-content-around">

                            {
                                ProductDetails?.images?.map(pimg => (
                                    ...
                                    <img src={`http://localhost:4000/${pimg}`} alt="" onClick={() => setimgDefault(pimg)} />
                                ))
                            }
                    ...
                    <div className="row product-content">
                        ...
                        <div className="price-discount">
                            <div className="price d-flex  ">
                                <p><strike>&#8377;{ProductDetails?.actual_price}</strike> <ins>&#8377;{ProductDetails?.price}</ins> <Button className="btn btn-primary">{ProductDetails?.discount}% off</Button></p>
                            </div>
                            <Button className="btn" onClick={() => onAddProduct(ProductDetails)} >ADD</Button>
